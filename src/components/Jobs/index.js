...
class Jobs extends Component {
  ...
  getJobsListContainer = () => {
    const {jobsList, locVal} = this.state
    const len = locVal.length
    const newJobsList =
      len > 0
        ? jobsList.filter(item => locVal.includes(item.location))
        : jobsList
    const count = newJobsList.length
    return count ? (
  ...
  onEmpChange = () => {
    ...
    const selectedEmpTypes = [...empTypeinputs]
  ...
  onLocChange = () => {
    const locTypeinputs = document.getElementsByName('locType')
    const selectedlocTypes = [...locTypeinputs]
      .filter(input => input.checked)
      .map(input => input.value)
    this.onLocTypeChange(selectedlocTypes)
  }
      ...
      <>
        ...
        <div className="hello-container">
          <div className="profile-items-container">
            ...
            <h1 className="li-heading">Locations</h1>
            <ul className="employment-type-container">
              {locationsList.map(item => (
                <li key={item.locationId} className="emp-li-item">
                  <input
